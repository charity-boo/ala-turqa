...
async function run() {
  ...
  const item = items.find(i => i.name === 'Shawarma Wrap + Fries');
  
  if (item) {
    try {
      // Use shawara fries.avif for this item since it features fries
      const imageUrl = await getDownloadUrl('shawara fries.avif');
    ...
    console.log(`Item 'Shawarma Wrap + Fries' not found.`);
  }
