...
async function run() {
  ...
  const cigBorekItems = items.filter(i => i.name.toLowerCase().includes('borek') || i.name.toLowerCase().includes('cig'));
  console.log("Found items:", cigBorekItems);

  if (cigBorekItems.length > 0) {
    try {
      const imageUrl = await getDownloadUrl('cig-borek.jpg');
      
      for (const item of cigBorekItems) {
        await db.collection('menu').doc(item.id).update({
          image: imageUrl,
          imageUrl: imageUrl
        });
        console.log(`Updated ${item.name} with cig-borek.jpg`);
      }
    } catch (e) {
      console.error("Failed to update image:", e.message);
    }
  }
