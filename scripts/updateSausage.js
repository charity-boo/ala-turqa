...
async function run() {
  ...
  const sausageItems = items.filter(i => 
    i.name.toLowerCase().includes('sausage') || 
    i.id.toLowerCase().includes('sausage')
  );
  
  console.log("Found sausage items:", sausageItems);

  if (sausageItems.length > 0) {
    try {
      const imageUrl = await getDownloadUrl('Sausage.jpg');
      
      for (const item of sausageItems) {
        await db.collection('menu').doc(item.id).update({
          image: imageUrl,
          imageUrl: imageUrl
        });
        console.log(`Updated ${item.name} with Sausage.jpg`);
      }
    } catch (e) {
