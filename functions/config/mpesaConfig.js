...
const mpesaConfig = {
  consumerKey: process.env.MPESA_CONSUMER_KEY ? process.env.MPESA_CONSUMER_KEY.trim() : '',
  consumerSecret: process.env.MPESA_CONSUMER_SECRET ? process.env.MPESA_CONSUMER_SECRET.trim() : '',
  // Store Number (Head Office) — used as BusinessShortCode for Buy Goods
  // Sandbox universal shortcode: 174379
  storeNumber: (process.env.MPESA_STORE_NUMBER || '174379').trim(),
  // Till Number — used as PartyB for Buy Goods (CustomerBuyGoodsOnline)
  tillNumber: (process.env.MPESA_TILL_NUMBER || process.env.MPESA_SHORTCODE || '').trim(),
  ...
  env: process.env.MPESA_ENV ? process.env.MPESA_ENV.trim() : 'sandbox'
};
